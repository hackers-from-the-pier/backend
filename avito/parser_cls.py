...
class TwoGisParser:
    ...
    async def check_address_async(self, address: str, session: aiohttp.ClientSession) -> Optional[str]:
        """
        Асинхронная проверка адреса в 2GIS
        """
        if not address:
            return None
    ...
    async def process_addresses_async(self, addresses: List[Dict[str, Any]]) -> Dict[str, str]:
        """
    ...
    async def process_report(self, report_id: int):
        """
        Обработка отчета и обновление данных в БД
        """
        async for session in get_async_session():
            try:
                # Получаем адреса из БД
                addresses = await get_commercial_addresses(report_id, session)
                if not addresses:
                    logger.error("Не найдены адреса для обработки")
                    return

                # Обрабатываем адреса
                results = await self.process_addresses_async(addresses)
                
                # Обновляем результаты в БД
                for address_data in addresses:
                    address = address_data.get('address')
                    client_id = address_data.get('id')
                    if address in results and results[address]:
                        await self.update_client_2gis_link(client_id, results[address], session)
                        logger.info(f"Обновлена ссылка 2GIS для клиента {client_id}")
...
if __name__ == '__main__':
    ...
    if proxy and "@" not in str(proxy):
        ...
        proxy = proxy_change_url = None
    else:
        logger.info("Используется SOCKS5 прокси с автоматической сменой IP")

    while True:
        try:
            # Сначала обрабатываем 2GIS
            if report_id:
                logger.info("Начинаем обработку 2GIS")
                two_gis_parser = TwoGisParser(
                    proxy=proxy,
                    proxy_change_url=proxy_change_url
                )
                asyncio.run(two_gis_parser.process_report(report_id))
                logger.info("Обработка 2GIS завершена")

            # Затем запускаем парсер Авито
            parser = AvitoParse(
                url=url,
                count=int(num_ads),
